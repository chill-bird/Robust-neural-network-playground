...
import pandas as pd
sys.path.insert(1, '/home/ed/robust-neural-network-playground/ui')
sys.path.insert(1, '/home/ed/robust-neural-network-playground/ui/graphics')
...
for i_episode in tqdm(range(num_episodes)):
    ...
    for t in count():
        ...
        #frames.append(frame)
        if(i_episode % 100 == 0):
            frame = render(float_state[0],float_state[2], 2*4.8)
            filename = "ui/frames/" + str(iterator) + ".png"
            frame.save(filename)
    ...
    if i_episode % TARGET_UPDATE == 0:
        target_net.load_state_dict(policy_net.state_dict())
    best_episode = max(best_episode, episode_durations[i_episode])
    #if(a!=best_episode):
        #for i in range(frame_count):
        #    _label_with_episode_number(frames[frame_count-i],i_episode)
    #    new_best = True
     #   a = best_episode
     #   print(a)
    """else:
        frame_files = frame_files[:-frame_count]"""
    
print('Complete')
env.close()
# Plot loss
# printing the list using loop
loss_history1 = pd.Series([float(x) for x in loss_history]).rolling(20).mean().tolist()
x = np.arange(len(loss_history1))
y = loss_history1
